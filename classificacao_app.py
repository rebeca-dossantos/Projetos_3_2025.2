...
# --- CARREGAR METRICS DE COMPARAÇÃO ---
print(">>> Carregando métricas de 'model_comparison_metrics.csv'...")
try:
    df_metrics = pd.read_csv('assets/model_comparison_metrics.csv')
    model_options = [{'label': model_name, 'value': model_name} for model_name in df_metrics['Modelo'].unique()]
...
# --- LISTAS DE COLUNAS / DICIONÁRIO DE TRADUÇÃO ---
float_cols = ['BMI']
...
print(">>> Calculando SHAP values globais (multiclasse) …")
explainer = shap.TreeExplainer(model)
shap_values_global_raw = explainer.shap_values(X_test_scaled_df)
#
# Verifica se é lista → multiclasse
if isinstance(shap_values_global_raw, list):
    # Cada elemento corresponde a uma classe
    shap_values_global = shap_values_global_raw
...
# --- 5. LAYOUT DO APP ---
app.layout = html.Div([
    html.H1("Classificação Multiclasse", style={'textAlign':'center'}),
    dcc.Tabs(id="tabs-main", value='tab-1', children=[
        dcc.Tab(label='Predição e Explicação Local', value='tab-1', children=[
            html.Div([
                ...
                html.Hr(),
                html.H2("Explicação Local (Gráficos de Força SHAP)", style={'color':'black'}),
                html.P("Este(s) gráfico(s) mostra(m) quais fatores estão 'empurrando' a previsão para cada classe."),
                html.Div(id='shap-local-forceplots')
            ], style={'padding':'20px'})
        ]),
        dcc.Tab(label='Explicações Globais', value='tab-2', children=[
            html.Div([
                ...
                html.P("Gráfico de barras mostrando o impacto médio de cada feature nas previsões para todas as classes."),
                ...
                html.P("Pontos vermelhos = Valor alto da feature; pontos azuis = Valor baixo."),
                html.Img(id='shap-global-beeswarm-plot', style={'width':'100%'})
            ], style={'padding':'20px'})
        ]),
...
# Callback para a PREDIÇÃO LOCAL e SHAP LOCAL (multiclasse)
@app.callback(
    [Output('output-previsao','children'),
     Output('shap-local-forceplots','children')],
...
def update_local_prediction(n_clicks, values, ids):
    ...
    proba = pipeline.predict_proba(input_scaled)[0]
    # Seleciona a classe com maior probabilidade
    class_idx = int(np.argmax(proba))
    prob_sel = proba[class_idx]
    resultado_texto = [
        html.Span("Classe prevista: ", style={'color':'black'}),
        html.Span(f"{class_idx} (Prob: {prob_sel*100:.1f}%)", style={'color':'red' if prob_sel<0.5 else 'green'})
    ]

    # Obter shap_values local: lista de arrays para cada classe
    shap_values_array = explainer.shap_values(input_scaled)
    # Esperado: expected_values corresponde à mesma ordem
    children_forceplots = []
    for cls_idx, class_shap_vals in enumerate(shap_values_array):
        ev = expected_values[cls_idx]
        shaps = class_shap_vals[0]
        ...
        html_str = f"<html><head>{shap.getjs()}</head><body><h4>Classe {cls_idx}</h4>{force_plot.html()}</body></html>"
        iframe = html.Iframe(srcDoc=html_str, style={'width':'100%','height':'250px','border':'none','marginBottom':'10px'})
        children_forceplots.append(iframe)

    return resultado_texto, children_forceplots

# Callback para o SHAP GLOBAL
@app.callback(
...
def update_global_shap_plot(tab_value):
    if tab_value == 'tab-2':
        # Cálculo da importância média absoluta considerando todas classes
        mean_abs_per_class = [np.abs(vals).mean(0) for vals in shap_values_global]
        # Soma ou média entre classes
        mean_abs_all = np.mean(np.vstack(mean_abs_per_class), axis=0)
        df_importance = pd.DataFrame({
            'Feature': features,
            'Importância Média (SHAP)': mean_abs_all
        ...
        fig_bar = px.bar(
            ...
            y='Feature',
            orientation='h',
            title='Top 20 Features que mais impactam a predição (todas as classes)'
        )
        fig_bar.update_layout(yaxis={'categoryorder':'total ascending'})

        # Beeswarm plot para multiclasse: usar todas as classes concatenadas ou escolher classe principal
        buf = io.BytesIO()
        plt.figure(figsize=(10,8))
        # Opção: concatenar todas as classes
        shap.summary_plot(
            np.vstack(shap_values_global),  # empilha
            pd.concat([X_test_scaled_df]*len(shap_values_global), ignore_index=True),
            show=False,
            max_display=20
        )
        plt.title("SHAP Beeswarm Plot (Todas as classes)", fontsize=16)
        plt.tight_layout()
...
# Callback para a MATRIZ DE CONFUSÃO
@app.callback(
...
def update_confusion_matrix_graph(tab_value, selected_model):
    if tab_value == 'tab-3' and selected_model:
        ...
        tp = model_data['TP']
        z = [[tn, fp],[fn, tp]]
        x_labels = ['Previsto: Classe 0','Previsto: Classe 1']
        y_labels = ['Real: Classe 0','Real: Classe 1']
        fig = px.imshow(
            z,
            x=x_labels,
            y=y_labels,
